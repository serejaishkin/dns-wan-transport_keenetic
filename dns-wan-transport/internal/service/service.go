...
func (a *App) Run() {
	...
	// Let health checker run once before accepting SOCKS connections
	// so we don't accept traffic with unknown backend state.
	for i := 0; i < 50 && !a.hc.IsHealthy(); i++ {
		// Wait up to 5s for first successful check
	}
