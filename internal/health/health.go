...
// Checker performs periodic health checks and reports status.
type Checker struct {
	cfg       config.Config
...
// Result is a single health check record.
type Result struct {
...
// New creates a Checker from config.
func New(cfg config.Config) (*Checker, error) {
	...
	// Select checker based on backend type
...
func (c *Checker) Run(ctx context.Context) {
	c.checkOnce() // immediate first check
...
func (c *Checker) LastCheck() time.Time {
	return c.lastCheck.Load().(time.Time)
}
...
// Latency returns the last check latency in ms.
func (c *Checker) Latency() int64 {
	return c.latency.Load()
}

// History returns the recent check results.
